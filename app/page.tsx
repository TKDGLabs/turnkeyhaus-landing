...
export default function Page() {
  ...
  return (
    <main className="bg-white text-[#0B0F0E] antialiased selection:bg-[#21c1a2]/30">
        ...
        <div className={`${shell} relative h-full flex flex-col justify-end pb-24`}>
          <motion.div initial="hidden" animate="visible" variants={reveal} className="max-w-5xl">
            ...
            <div className="mt-16 flex flex-col lg:flex-row lg:items-end justify-between gap-12">
              <p className="max-w-[420px] text-lg leading-relaxed text-white/60 lg:text-xl font-medium">
                {content.heroValue.body}
              </p>
              <div className="flex gap-4 items-center">
                <ActionLink href="#pilot" className="rounded-full bg-[#21c1a2] px-10 py-5 text-[15px] font-black text-black hover:scale-105 active:scale-95">
      ...
      {/* 3. Stats Overlay (Floating on White) */}
      ...
      <section id="problem" className="py-32 lg:py-52 bg-white">
        <div className={shell}>
      ...
      {/* 5. Team: Real Roles & Pure Visuals */}
      <section id="team" className="py-32 lg:py-52 bg-white border-t border-black/5">
        <div className={shell}>
          ...
          <div className="grid gap-12 lg:grid-cols-3">
            {content.leadership.people.map((person, idx) => (
              <motion.div key={person.name} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal} transition={{ delay: idx * 0.2 }} className="group">
                <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-[#f8f9fa] grayscale transition-all duration-700 group-hover:grayscale-0">
                ...
                <div className="mt-12">
                  <h3 className="text-3xl font-bold tracking-tight mb-2">{person.name}</h3>
                  <div className="flex flex-wrap gap-x-3 text-[14px] font-black text-[#21c1a2] uppercase tracking-widest">
                    {/* Lead 대신 실제 맡고 있는 역할 노출 */}
                    {person.responsibilities.slice(0, 2).map(r => <span key={r}>{r}</span>)}
                  </div>
          ...
          <div className="grid gap-4 lg:grid-cols-12">
            {content.servicePillars.cards.map((card, idx) => (
              ...
              >
                ...
                {idx === 0 && (
                   <div className="hidden lg:flex items-center justify-center opacity-20 group-hover:opacity-100 transition-opacity duration-1000">
      ...
      {/* 8. Portfolio Section */}
      <section id="portfolio" className="py-32 lg:py-52 bg-white">
        <div className={shell}>
          <div className="mb-32">
            <h2 className="text-[48px] font-bold tracking-tighter lg:text-[84px] leading-none mb-12">{content.portfolio.h2}</h2>
            <p className="text-2xl text-black/30 font-medium max-w-2xl italic leading-relaxed">{content.portfolio.lead}</p>
          </div>
            ...
            {content.portfolio.items.map((item, i) => (
              ...
              >
                ...
                <div className="space-y-10">
                  ...
                  <h3 className="text-5xl lg:text-6xl font-bold tracking-tighter leading-[1.1]">{item.title}</h3>
        ...
        <div className={shell}>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-12 border-t border-white/10 pt-20">
            <div>
              ...
              <div className="space-y-2 text-sm text-white/20 font-medium">
                 {content.footer.lines.map(line => <p key={line.label}>{line.label}: {line.value}</p>)}
              </div>
            </div>
            <div className="flex flex-wrap gap-12 text-[11px] font-black tracking-[0.3em] text-white/20">
